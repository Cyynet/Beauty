...
-(void)showInRect:(CGRect)rect
{
    ...
    view.frame = CGRectMake(0,CGRectGetMaxY(selectTimeIsNotLegalLabel.frame) + 5, MYScreenW - MYMargin, 40);
    ...
    rightBtn.titleLabel.font = leftFont;
...
- (void)sure
{
    if ([selectTimeIsNotLegalLabel.text isEqualToString: @"温馨提示：所选时间不合法，无法提交(10:00-18:00)"]){
        return;
    }else{
    ...
    if ([self.delegate respondsToSelector:@selector(datePickView:showTimeLabelText:)]) {
        [self.delegate datePickView:self showTimeLabelText:nowPickerShowTimeLabel.text];
    }
}
...
{
    nowPickerShowTimeLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 10.0, MYScreenW - MYMargin, 18)];
    ...
    nowPickerShowTimeLabel.text = [NSString stringWithFormat:@"%@-%02ld-%02ld %@:%@:%@",[dateString substringWithRange:NSMakeRange(0, 4)],(long)monthInt,(long)dayInt,[dateString substringWithRange:NSMakeRange(8, 2)],[dateString substringWithRange:NSMakeRange(10, 2)],[dateString substringWithRange:NSMakeRange(12, 2)]];
    [self.contentView addSubview:nowPickerShowTimeLabel];
    
    timeBroadcastView = [[UIView alloc] initWithFrame:CGRectMake(kMargin - 3, 40, self.width - 2 * MYMargin, 190.0)];
    ...
    [self.contentView addSubview:timeBroadcastView];
    ...
    UIView *middleSepView = [[UIView alloc] initWithFrame:CGRectMake(0, 75, MYScreenW - 2 * MYMargin, 38)];
    [middleSepView setBackgroundColor:MYColor(193, 177, 122)];
    ...
    selectTimeIsNotLegalLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(timeBroadcastView.frame) + 5, MYScreenW - MYMargin, 15)];
    [selectTimeIsNotLegalLabel setBackgroundColor:[UIColor clearColor]];
    [selectTimeIsNotLegalLabel setFont:[UIFont fontWithName:@"SYFZLTKHJW--GB1-0" size:11.0]];
    [selectTimeIsNotLegalLabel setTextAlignment:NSTextAlignmentCenter];
    [self.contentView addSubview:selectTimeIsNotLegalLabel];
}
//设置年月日时分的滚动视图
...
- (void)scrollviewDidChangeNumber
{
    ...
    NSInteger minuteInt = minuteLabel.tag - 1;
    NSInteger secondInt = secondLabel.tag - 1;
    nowPickerShowTimeLabel.text = [NSString stringWithFormat:@"%ld-%02ld-%02ld %02ld:%02ld:%02ld",(long)yearInt,(long)monthInt,(long)dayInt,(long)hourInt,(long)minuteInt,(long)secondInt];
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    ...
    NSArray *array = [nowPickerShowTimeLabel.text componentsSeparatedByString:@" "];
    NSInteger hour = [[array[1] substringWithRange:NSMakeRange(0, 2)] intValue];
    if (NSOrderedAscending == [selectDate compare:nowStrDate]) {//选择的时间与当前系统时间做比较
        ...
        selectTimeIsNotLegalLabel.text = @"温馨提示：所选时间不合法，无法提交(10:00-18:00)";
